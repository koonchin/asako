  ...
  return (
    <div className="fixed bottom-8 right-24 z-[90] flex flex-col items-end"> {/* ขยับซ้ายมาจาก right-8 เดิมของ AgriAdvisor */}
      ...
      <button
        ...
        className={`w-14 h-14 rounded-full shadow-xl flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 z-50 ${isOpen ? 'bg-gray-900 rotate-45' : 'bg-red-600'}`}
      >
        {isOpen ? (
           <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg> // กากบาท (จากบวกหมุน 45 องศา)
        ) : (
